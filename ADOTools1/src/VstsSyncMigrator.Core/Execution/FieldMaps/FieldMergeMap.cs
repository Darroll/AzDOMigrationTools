...
namespace VstsSyncMigrator.Engine.ComponentContext
{
    public class FieldMergeMap : FieldMapBase
    {
        ...
        private static readonly Lazy<TraceSource> _mySource = new Lazy<TraceSource>(() => Tracing.Create("Migration.Engine.FieldMergeMap"));

        #endregion

        #region - Private Members

        private readonly FieldMergeMapConfig _config;
        ...
        protected override void InternalExecute(WorkItem sourceWI, WorkItem targetWI)
        {
            ...
            string verb;
            string fieldValue;
            string[] sourceFields = new string[] { _config.SourceField1, _config.SourceField2 };
            ...
            if (process)
            {
                ...
                // Send some traces.
                sourceFields = new string[] { _config.SourceField1, _config.SourceField2 };
                _mySource.Value.TraceInformation("[{0}] field merged {1}:{2} to {3}:{4}", verb, sourceWI.Id, string.Join("+", sourceFields), targetWI.Id, _config.TargetField);
            }
            else
            {
                // Send some traces.
                sourceFields = new string[] { _config.SourceField1, _config.SourceField2 };
                _mySource.Value.TraceInformation("[{0}] field already merged {1}:{2} to {3}:{4}", verb, sourceWI.Id, string.Join("+", sourceFields), targetWI.Id, _config.TargetField);
            }
        ...
        {
            get { return $"{_config.SourceField1}/{_config.SourceField2} -> {_config.TargetField}"; }
        }
