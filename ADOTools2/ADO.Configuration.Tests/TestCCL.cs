...
namespace ADO.Configuration.Tests
{
    ...
    public class TestCcl
    {
        ...
        public static void MyClassInitialize(TestContext testContext)
        {
            ...
            configRoot = "CCL";
            sourcePatToken = "Environment.PatTokenCCL";
            ...
            importSourceCodeCredentialsPassword = "Environment.PatTokenCCL";
            importSourceCodeCredentialsGitPassword = "Environment.GitHubPassword";
            destinationCollection = "ccldevopsabcs";
            int projectNumber = 5;
            destinationProject = $"OneProject_{projectNumber.ToString().PadLeft(2, '0')}";
            destinationProjectProcessName = "CCL-One";
            //used for Bulk Import
            destinationProcessTypeName = "Agile";// "MigrateableAgile";
            defaultTeamName = $"{destinationProject} Team";
            description = "One Project to Rule Them All";
            reflectedWorkItemId = "ReflectedWorkItemId";
            numberOfDaysToImport = 90;
            ...
            bool treeAlreadyCreated = File.Exists($"..\\..\\Input\\{configRoot}\\businessNodeFromCsv2.json");
            if (treeAlreadyCreated)
            {
                root = SimpleMutableBusinessNodeNode.LoadFromJson($"..\\..\\Input\\{configRoot}\\businessNodeFromCsv2.json");
                //modify root Name
                root.Item.Name = destinationProject;
                //back to dataNode
                BusinessNodeDataNode businessNodeDataNode = root.ToBusinessNodeDataNodeRecursive();
                //save back
                root.SaveToJson($"..\\..\\Input\\{configRoot}\\businessNodeFromCsvNEW2.json");
            }
        }

        [TestMethod, Ignore] //use Test_do_migration_CCL INSTEAD
        public void TestGenerateBusinessNodeFromCsv()
        {
            List<BusinessHierarchyCsv> businessHierarchyCsvList;
            BusinessNode businessNode
                    = BusinessNode.LoadFromCsv($"..\\..\\Input\\{configRoot}\\businessHierarchyCsv2.csv",
            ...
            DirectoryInfo di = new DirectoryInfo($"..\\..\\Output\\{configRoot}");
            if (!di.Exists)
                Directory.CreateDirectory(di.FullName);
            string json = JsonConvert.SerializeObject(businessNode, Formatting.Indented);
            File.WriteAllText(Path.Combine(di.FullName, "businessNodeFromCsv2.json"), json);
            //also write to Input folder
            File.WriteAllText(Path.Combine($"..\\..\\Input\\{configRoot}", "businessNodeFromCsv2.json"), json);
